...
def get_projects(gitlab_url, token):
    """Fetch all projects from the GitLab server, sorted by last_activity_at."""
    projects = []
    page = 1
    while True:
        url = f"{gitlab_url}/api/v4/projects?order_by=last_activity_at&sort=desc&per_page=100&page={page}"
...
def main():
    ...
    # Select the 15 most recently used projects
    latest_projects = sorted(old_projects, key=lambda p: p['last_activity_at'], reverse=True)[:15]
