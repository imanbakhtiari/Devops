...
OLD_GITLAB_TOKEN = "gitlab-old-token"
NEW_GITLAB_TOKEN = "gitlab-new-token"
...
def import_variables_to_new_server(project_path, variables):
    """Import variables to a new GitLab server and overwrite existing ones if necessary."""
    ...
    for var in variables:
        url = f"{NEW_GITLAB_URL}/api/v4/projects/{project_id}/variables/{var['key']}"
        data = {
            "key": var["key"],
            "value": var["value"],
        ...
        response = requests.post(f"{NEW_GITLAB_URL}/api/v4/projects/{project_id}/variables",
                                 headers=get_headers(NEW_GITLAB_TOKEN),
                                 json=data)
        ...
        if response.status_code == 201:
            ...
            # Variable already exists, attempt to update
            response = requests.put(url, headers=get_headers(NEW_GITLAB_TOKEN), json=data)
            if response.status_code == 200:
                print(f"Updated variable {var['key']} in project {project_path}")
            else:
                print(f"Failed to update variable {var['key']} in project {project_path}: {response.text}")
        else:
